 ...
 *
 *   $Id: m_away.c,v 1.4 2005/08/27 16:23:50 jpinto Exp $
 */
               ...
               char *parv[])
{
  ...
  {
    aConfItem* msgv;
    char* stripedmsg = strip_control_codes_lower(awy2);
    if(!IsOper(sptr) && (msgv = is_msgvlined(stripedmsg)))
    {
      sendto_one(sptr,":%s NOTICE %s :%s", me.name, parv[0], msgv->passwd);
      return 0;
    }
  }
  ...
  stripedmsg = strip_control_codes_lower(awy2);
