 ...
 *
 *   $Id: flud.c,v 1.4 2005/08/27 16:23:49 jpinto Exp $
 */
#include "flud.h"
#include "client.h"
                            ...
                            int type)           /* for future use */
{
  ...
  sendto_ops_imodes(IMODE_BOTS, "Flooder %s [%s@%s] on %s target: %s",
...
{               
  ...
  ** and last_msg members.  Also make a running total count */
  ...
  if(!found)
    {
      if((current = MyMalloc(sizeof(struct fludbot))) != NULL)
        {
          ...
          if((newfludee = MyMalloc(sizeof(struct SLink))) != NULL)
            {
              if(cptr)
                {
                  newfludee->flags = 0;
