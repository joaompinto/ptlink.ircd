 ...
 *
 *   $Id: parse.c,v 1.5 2005/08/27 16:23:50 jpinto Exp $
 */
#include "parse.h"
#include "channel.h"
#include "client.h"
...
void init_tree_parse(struct Message *mptr)
{
  ...
  if (mpt->cmd)
    {
      irclog(L_CRIT, "bad msgtab entry: ``%s''\n", mpt->cmd);
      exit(1);
    }
}
  ...
  if (IsServer(sptr) || IsMe(sptr))
    {
      ...
      sendto_ops_imodes(IMODE_DEBUG, "Message for %s[%s] from %s",
                   ...
                   char *parv[])
{
  ...
  */
  ...
  if (parc > 1)
    ...
    {
      ts_warn("Got empty numeric %s from %s", numeric, parv[0]);
      return 0;
    }
