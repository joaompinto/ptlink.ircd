 ...
 *
 *   $Id: s_serv.c,v 1.6 2005/08/27 16:23:50 jpinto Exp $
 */
#include "s_serv.h"
#include "msg.h"		
#include "channel.h"
...
#include "scache.h"
#include "send.h"
#include "m_gline.h" 	/* for send_all_glines() */
#include "sxline.h" 	/* for send_all_sxlines() */
#include "sqline.h"		/* for send_all_sqlines() */
#include "svline.h"		/* for send_all_svlines() */
#include "vlinks.h"		/* for send_all_vlinks() */
#include "m_svsinfo.h"	/* for my_svsinfo_ts */ 
#include "s_debug.h"
...
#ifndef NEG_PORT
  /* The negative ports kludge allows you to specify ports for every
  ** server, just not autoconnect to each one.  This message can get QUITE
  ** annoying when using this feature, so just hide it -Hwy
  */
  ...
  if (connecting)
    {
      ...
      if (!(con_conf->flags & CONF_FLAGS_ALLOW_AUTO_CONN))
        {
#ifndef NEG_PORT
/* Same as above...hide these when using NEG_PORT */
        
#ifdef HIDE_SERVERS_IPS
          sendto_realops("Connection to %s not activated, autoconn is off.",
	             con_conf->name);
	  sendto_realops("WARINING AUTOCONN on %s is disabled",
	             con_conf->name);
#else
...
void send_capabilities(struct Client* cptr, int use_zip)
{
  ...
  if(HostPrefix)
    {
  	strcat(msgbuf,"PREFIX=");
  	strcat(msgbuf,HostPrefix);
    }
...
static void sendnick_TS(struct Client *cptr, struct Client *acptr)
{
  ...
  if (IsPerson(acptr))
    {
      send_umode(NULL, acptr, 0, ALL_UMODES, ubuf);
      if (!*ubuf)
	  ...
	  if (acptr->user->vlink)
	    sendto_one(cptr, "NNICK %s %d %lu %s %s %s %s %s!%s :%s",
	    		acptr->name, acptr->hopcount+1, acptr->tsinfo, ubuf,
	    		acptr->username, acptr->realhost, acptr->host,
	    		acptr->user->server, acptr->user->vlink->name, 
	    		acptr->info);
	  else
	    sendto_one(cptr, "NNICK %s %d %lu %s %s %s %s %s :%s",
			acptr->name, acptr->hopcount+1, acptr->tsinfo, ubuf,
  			acptr->username, acptr->realhost, acptr->host,
...
int server_estab(struct Client *cptr, char* remversion)
{
  ...
  if (!(n_conf = find_conf_name(cptr->confs, host, CONF_NOCONNECT_SERVER)))
    {
      ...
      irclog(L_NOTICE, "Access denied. No N line for server %s", inpath_ip);
  ...
  if (!(c_conf = find_conf_name(cptr->confs, host, CONF_CONNECT_SERVER )))
    {
      ...
      irclog(L_NOTICE, "Only N (no C) field for server %s",inpath_ip);
  ...
  if (*n_conf->passwd && 0 != strcmp(n_conf->passwd, encr))
    {
      ...
      irclog(L_NOTICE, "Access denied (passwd mismatch) %s", inpath_ip);
  ...
  if (IsUnknown(cptr))
    {
      ...
      */
      ...
      sendto_one(cptr, "SERVER %s 1 %s :%s",
                 my_name_for_link(me.name, n_conf), ircdversion,
...
#endif /* ZIP_LINKS */
    sendto_one(cptr,"SVINFO %d %d %d", TS_CURRENT, TS_MIN, CurrentTime);
	sendto_one(cptr,"SVSINFO %lu %d", my_svsinfo_ts, Count.max_tot);
#ifdef SCS
  scs_timeout_scheck();
  if (IsCapable(cptr, CAP_SCS))
    sendto_one(cptr,"SCSCHK %lu %i", our_cache_ts, cache_count);
#endif
  ...
  */
  ...
  if (!strcasecmp(cptr->name,ServicesServer))
    {
      SetService(cptr);
      services_on = -1;
      SetServicesPath(cptr);
      sendto_ops("Services connected");
    }
  ...
  irclog(L_NOTICE, "Link with %s established: (%s) link",
  ...
  if( remversion ) /* saver server version */
	strncpy_irc(cptr->serv->version, remversion, HOSTLEN);
  ...
  for(acptr=serv_cptr_list;acptr;acptr=acptr->next_server_client)
    {
      ...
      sendto_one(acptr,":%s SERVER %s 2 %s :%s",
                 me.name, cptr->name, cptr->serv->version, cptr->info);
    }
  ...
  */
  ...
  for (acptr = &me; acptr; acptr = acptr->prev)
   ...
   if(!IsService(cptr))	/* services dont have no use for our sq/sg/sv/glines */
	{
	  send_all_sqlines(cptr);	/* propagate sqlines */
	  send_all_sxlines(cptr);	/* propagate sxlines */	  
	  send_all_svlines(cptr);	/* propagate svlines */
      send_all_vlinks(cptr);	/* propagate vlinks */      
	  send_all_glines(cptr);	/* propagate glines */
	}	
