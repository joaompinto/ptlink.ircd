...
*
*   $Id: whowas.c,v 1.3 2005/08/27 16:23:50 jpinto Exp $
*/
#include "whowas.h"
#include "client.h"
#include "common.h"
...
void add_history(aClient* cptr, int online)
{
  ...
  assert(sizeof(cptr->realhost) == sizeof(who->realhost));
  ...
  strcpy(who->realhost, cptr->realhost);
      ...
      if (!irccmp(nick, temp->name))
        {
          sendto_one(sptr, form_str(RPL_WHOWASUSER),
                     ...
                     temp->username,
					 IsOper(sptr) ? temp->realhost: temp->hostname,
                     temp->realname);
          sendto_one(sptr, form_str(RPL_WHOISSERVER),
                     ...
                     temp->name,
#ifdef SERVERHIDE
                     IsAnOper(sptr) ? temp->servername : NetworkName,
#else
                     temp->servername,
#endif
                     myctime(temp->logoff));
