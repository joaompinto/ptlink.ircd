 ...
 *
 *   $Id: m_server.c,v 1.5 2005/08/27 16:23:50 jpinto Exp $
 */
 ...
 */
  ...
  if ((acptr = find_server(host)))
	...
	return exit_client(bcptr, bcptr, &me, "Server Exists");
      ...
      sendto_realops("Link %s cancelled, server %s reinstroduced by %s",
  ...
  if (IsServer(cptr))
	...
	strncpy_irc(acptr->serv->version, remversion, HOSTLEN);
		
      SetServer(acptr);
	  
      if (!strcasecmp(acptr->name,ServicesServer))
        {
	  SetService(acptr);
	  services_on = -1;
      ...
      for (bcptr = serv_cptr_list; bcptr; bcptr = bcptr->next_server_client)
	    ...
	    sendto_realops("Lost N-line for %s on %s. Closing",
          ...
          sendto_one(bcptr, ":%s SERVER %s %d %s :%s",
                     parv[0], acptr->name, hop + 1, acptr->serv->version, acptr->info);
                         
        }
      
      sendto_ops_imodes(IMODE_EXTERNAL, "Server %s being introduced by %s",
  ...
  if (check_server(cptr))
    return server_estab(cptr, remversion);
