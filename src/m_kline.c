 ...
 *
 *   $Id: m_kline.c,v 1.4 2005/08/27 16:23:50 jpinto Exp $
 */
  ...
  if( (aconf = is_klined(host,user,(unsigned long)ip)) )
     {
       ...
       if( aconf->status & CONF_KILL )
         {
           kreason = aconf->passwd ? aconf->passwd : "<No Reason>";
#ifdef SLAVE_SERVERS
           if(!IsServer(sptr))
#endif
             sendto_one(sptr,
...

{
  ...
  if(!is_address(host,&ip_host,&ip_mask))
    {
      ...
      *p++ = '\0';
      ...
      ip_mask = 0xFFFFFF00L;
      ip_host = ntohl((unsigned long)acptr->ip.S_ADDR);
    }
  ...
  if( (aconf = match_Dline(ip_host)) )
     {
       char *dreason;
       dreason = aconf->passwd ? aconf->passwd : "<No Reason>";
