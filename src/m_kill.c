 ...
 *
 *   $Id: m_kill.c,v 1.4 2005/08/27 16:23:50 jpinto Exp $
 */
...
#include "send.h"
#include "whowas.h"
#include "dconf_vars.h"	/* EnableSelfKill */
...
int m_kill(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  if (!EnableSelfKill && (sptr == acptr))
  {
    sendto_one(sptr,
      ":%s NOTICE %s :Suicide is not allowed on this net", me.name,  parv[0]);
    return 0;
  }
  ...
  if(!IsService(sptr))
	{
       ...
       */
  		...
  		sendto_ops_imodes(IMODE_KILLS,
                     "Received KILL message for %s. Path: %s!%s",
