 ...
 *
 *   $Id: m_squit.c,v 1.4 2005/08/27 16:23:50 jpinto Exp $
 */
...
int m_squit(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
  ...
  */
  ...
  if (MyClient(sptr) && IsService(acptr) && !IsSAdmin(sptr))
	{
	  sendto_one(sptr, form_str(ERR_NOPRIVILEGES), me.name, parv[0]);
      return 0;	
	}
  ...
  if (MyClient(sptr) && !IsOperRemote(sptr) && !MyConnect(acptr))
   ...
   /* This code is stupid, but works.. need to be done by someone smarter
   * I don't know why, but now I really hate those vlinks :)
   * BTW, IsMe(acptr) didn't made the same result here.  
   *  -- openglx
   */
  if (MyClient(sptr) && strcasecmp(acptr->name, me.name) == 0) 
     {
      sendto_one(sptr,":%s NOTICE %s :Hey, you are trying to SQUIT me!",me.name,parv[0]);
      return 0;
     }
  ...
  if (MyConnect(acptr) && !IsAnOper(cptr))
    {
      ...
      irclog(L_TRACE, "SQUIT From %s : %s (%s)", parv[0], server, comment);
    }
