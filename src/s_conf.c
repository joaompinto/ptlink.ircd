 ...
 *
 *  $Id: s_conf.c,v 1.10 2005/08/27 16:23:50 jpinto Exp $
 */
#include "s_conf.h"
#include "channel.h"
#include "class.h"
...
#ifdef IPV6
struct SOCKADDR_IN ipv6localhost;
#endif
...
static  int  oper_imodes_from_string(char *);
...
#endif
{
  ...
  if (reply)
#ifdef IPV6
    bcopy(reply->hp->h_addr, (char*)aconf->ipnum.S_ADDR, sizeof(struct in6_addr));
#else
...
#ifdef IPV6
   /* IPV6 DOESN't WORK WITH ADNS -FIX- 
    memcpy(aconf->addr.S_ADDR, reply->rrs.addr->addr.inet.sin_addr.S_ADDR, sizeof(struct IN_ADDR));
    */
#else  
    ...
    if (tmp->status & tmpmask)
      {
        ...
        if(mask & (CONF_CONNECT_SERVER|CONF_NOCONNECT_SERVER))
        ...
        else if(mask & (CONF_OPERATOR|CONF_LOCOP))
          {
      	  ...
      	  {
        	if (!find_server(name))
          	  sendto_one(sptr, ":%s 247 %s X %s %d", me.name,
                            sptr->name, name, port);
      	  }		  
        else
          sendto_one(sptr, form_str(p->rpl_stats), me.name,
...
int check_client(struct Client *cptr,char *username,char **reason)
{
  ...
  ClearAccess(cptr);
    ...
    {	
	  if(i==-1)
		{
    	  irclog(L_INFO, "Access denied: %s[%s]", cptr->name, sockname);
		  if(RandomHost)
	  ...
	  if (((i == -3) || (i==-5)) && CheckClones)
    	remove_clone_check(cptr);
		 
      return i;
    } else {
		if(CheckClones && (((cptr->listener->options & LST_WEBCHAT)==0) ||
#ifdef IPV6		
			!IN6_IS_ADDR_LOOPBACK(&cptr->ip)) && 
#else			
			(inet_netof(cptr->ip) != IN_LOOPBACKNET)) && 
#endif
			check_clones(cptr, cptr->name))
		{
			aconf = make_conf();
			DupString(aconf->name, cptr->realhost);
			aconf->hold = CurrentTime + CheckClonesDelay;
			DupString(aconf->passwd, "Too many connections from your host, please try again later");
		    aconf->next = zlines;
			zlines = aconf;		
...
verify_access(struct Client *cptr, const char* username, char **preason)
{
  ...
  if (IsGotId(cptr))
      ...
      if(aconf && !IsConfElined(aconf))
        {
          if( (tkline_conf = find_tkline(cptr->host, 
			  IsGotId(cptr) ? cptr->username : username, ntohl((unsigned long)cptr->ip.S_ADDR))))
            aconf = tkline_conf;
        }
  ...
  else
    {
      ...
      if(aconf && !IsConfElined(aconf))
        {
          if( (tkline_conf = find_tkline(cptr->host, non_ident, ntohl((unsigned long)cptr->ip.S_ADDR))) )
            aconf = tkline_conf;
        }
    }
  ...
  if(aconf && (aconf->status & CONF_CLIENT))
    {
          gkill_conf = find_zline(cptr->realhost);  /* try to match hostname*/
          
          if(!gkill_conf && !IPIdentifyMode) /* try to match numeric IP */
                  gkill_conf = find_zline((char*) inetntoa((char *) &cptr->ip));
          
    	  if (gkill_conf)
      		{
        	  *preason = gkill_conf->passwd ? gkill_conf->passwd : "No reason";                  
        	  sendto_one(cptr, ":%s NOTICE %s :*** Z-lined",
              	  me.name, cptr->name);
          ...
          if ( !IsConfElined(aconf) && !IsConfExemptGline(aconf) )
            {
			  ...
			  else if((gkill_conf = is_sxlined(cptr->info)))
				{
                  *preason = gkill_conf->passwd ? gkill_conf->passwd : "No reason";                  
                  sendto_one(cptr, ":%s NOTICE %s :*** G-lined",
                             me.name,cptr->name);
...
#endif
{
  unsigned long ip_in=(unsigned long)cptr->ip.S_ADDR;        
#ifdef LIMIT_UH
  Link *new_link;
#endif
...
struct ConfItem *find_me()
{
  ...
  irclog(L_CRIT, "Server has no M: line");
  exit (-1);
...
{
  ATline *ntline, *tline = tlines;
  char lpath[256];
  
  snprintf(lpath, 256, "%s/ircd.log", ConfigFileEntry.varpath);    
  if (sig)
    {
      sendto_realops("Got signal SIGHUP, reloading ircd conf. file");
      irclog(L_NOTICE, "Got signal SIGHUP, reloading ircd conf. file");
    }

  /* Lets clear the T:lines list */
  while(tline)
	{
          ...
          if(!ircncmp(line+1,"include ",8))
            {
              if( (filename = strchr(line+8,'"')) )
                ...
                {
                  irclog(L_ERROR, "Bad config line: %s", line);
                  continue;
                }
...
/* later          
          if(tmp[1]=='s')
            aconf->flags |= CONF_FLAGS_SSL_LINK;
*/
          break;
        case 'c':
          ccount++;
        ...
        default:
          irclog(L_ERROR, "Error in config file: %s", line);
          break;
        }
      if (IsIllegal(aconf)) 
        {
          ...
          if(aconf->status & CONF_OPERATOR)
          ...
          else if(aconf->status & CONF_CONNECT_SERVER)
            {
              if ((tmp = getfield(NULL)) == NULL)
                break;
              aconf->port = atoi(tmp);
              if (aconf->port && (aconf->port < 0))
                {
                  aconf->flags &= ~CONF_FLAGS_ALLOW_AUTO_CONN;
                  aconf->port = abs(aconf->port);
                }
            }
#endif            
          else
	  ...
	  /* class field */
          ...
          if(aconf->status & (CONF_LOCOP|CONF_OPERATOR))
            {
              if ((tmp = getfield(NULL)) == NULL) /* get umodes */
                break;
              /* ugly hack alert, we use ->hold to store conf umodes */
              aconf->hold = oper_flags_from_string(tmp);
              
              if ((tmp = getfield(NULL)) == NULL) /* get imodes */
                break;
              /* ugly hack alert, we use ->ip to store conf imodes */
              aconf->ip = oper_imodes_from_string(tmp);              
            }
          else if(aconf->status & CONF_CONNECT_SERVER)
            {
              if ((tmp = getfield(NULL)) == NULL)
      ...
      if ( aconf->status & CONF_LISTEN_PORT)
        {
          ...
          if((aconf->passwd[0] == '\0') || (aconf->passwd[0] == '*'))
            add_listener(aconf->port, NULL , aconf->user, aconf->host);
          else
            add_listener(aconf->port, (const char *)aconf->passwd, aconf->user, aconf->host);
          free_conf(aconf);
          aconf = NULL;
          continue;
        }
      ...
      if (aconf->status & CONF_SERVER_MASK)
        {
          ...
          if (BadPtr(aconf->passwd))
            {
              irclog(L_ERROR, "Bad C/N line passwd for %s");
          ...
          if( SplitUserHost(aconf) < 0 )
            {
              irclog(L_ERROR, "Bad C/N line user/host portion for %s");
      ...
      */
      if (aconf->status == CONF_ME)
      ...
      else if (aconf->host && (aconf->status & CONF_CLIENT))
        {
	      ...
	      if(CheckIdentd)
           ...
           if( is_address(aconf->host,&ip_host,&lip_mask) )
	     {
               aconf->ip = ip_host & lip_mask;
               aconf->ip_mask = lip_mask;
               add_ip_Iline( aconf );
             }
           else
	     {
	  ...
	  else if (aconf->status & CONF_TLINE)
		{
			ATline* newt;
			dontadd = 1;	
			newt = (ATline*) MyMalloc(sizeof(ATline));
			newt->host = aconf->host;			
			InitMessageFile(USER_MOTD, aconf->passwd, &newt->motd);
			if (ReadMessageFile(&newt->motd) == -1)
			  {
				irclog(L_ERROR,"Error opening T:line motd file %s", aconf->passwd);			
				MyFree(newt);
				break;
			  }
			newt->next = tlines;
  ...
  for( ; include_list; include_list = nextinclude )
    {
      ...
      if ((file = openconf(include_list->name)) == 0)
        ...
        {
          sendto_ops_imodes(IMODE_DEBUG, "Hashing in %s include file",include_list->name);
          initconf(file, NO);
        }
...
static void lookup_confhost(struct ConfItem* aconf)
{
  ...
  if (BadPtr(aconf->host) || BadPtr(aconf->name))
    {
      irclog(L_ERROR, "Host/server name error: (%s) (%s)",
...
struct ConfItem *find_kill(aClient* cptr)
{
  ...
  return (IsElined(cptr)) ? 0 : find_is_klined(cptr->realhost, 
                                               cptr->username, 
  ...
  while(*privs)
    {
      if(*privs == 'O')                     /* allow global kill */
        ...
        int_privs |= CONF_OPER_ADMIN;      	/* oper is server admin */				
      privs++;
    }
  return(int_privs);
}
...
static int oper_flags_from_string(char *flags)
{
  ...
  while(*flags)
    {
      int_flags |= user_modes_from_c_to_bitmask[(int) *flags];		
      flags++;
    }
  int_flags &= ~INTERNAL_UMODES; /* remove any services only mode */
  return(int_flags);
}

/* oper_imodes_from_string
 *
 * inputs        - imodes as string
 * output        - imodes as bit mask
 * side effects -
 *
 * -Lamego
 */

static int oper_imodes_from_string(char *imodes_s)
{
  int i;
  int imodes = 0;
  ...
  while(*imodes_s)
    {
      i = 0;
      while(imodes_map[i].mode && (*imodes_s != imodes_map[i].letter))
        ++i;
      imodes |= imodes_map[i].mode;
      imodes_s++;
    }
  return imodes;
}
...
char *oper_flags_as_string(int flags)
{
  ...
  *flags_ptr = '\0';
  ...
  for (i = 0; user_modes[i].letter; i++)
    if (flags & user_modes[i].mode)
      *flags_ptr++  = user_modes[i].letter;
  ...
  if ((file = openconf(filename)) == 0)
    {
      if(cold)
        {
          irclog(L_CRIT, "Failed in reading configuration file %s", filename);
          exit(-1);
        }
      else
        {
      ...
      if ((file = openconf(kfilename)) == 0)
        {
          if(cold)
            {
              irclog(L_ERROR,"Failed reading kline file %s", kfilename);
            }
          else
            {
      ...
      if ((file = openconf(dfilename)) == 0)
        {
          if(cold)
            {
              irclog(L_ERROR,"Failed reading dline file %s", dfilename);
            }
          else
            {
...
void write_kline_or_dline_to_conf_and_notice_opers(
  ...
  if(type==KLINE_TYPE)
    irclog(L_TRACE, "%s added K-Line for [%s@%s] [%s]",
        sptr->name, user, host, reason);
  else
    irclog(L_TRACE, "%s added D-Line for [%s] [%s]",
           sptr->name, host, reason);
}
...
static char *getfield(char *newline)
{
  ...
  field = line;
  /* Have we got a v6 æddress encoded in a field? */
#ifdef IPV6
  if(line[0] == '[')
  {
    line++;
	field++;
    if((end = strchr(line, ']')) == (char *)NULL)
    {
      line = (char *)NULL;
      if ((end = strchr(field,'\n')) == (char *)NULL)
