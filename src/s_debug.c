 ...
 *
 *   $Id: s_debug.c,v 1.5 2005/08/27 16:23:50 jpinto Exp $
 */
#include "s_debug.h"
#include "channel.h"
#include "class.h"
...
#include "send.h"
#include "struct.h"
#include "m_gline.h" /* for gline mem usage */
...
void count_memory(aClient *cptr,char *nick)
{
  ...
  u_long lmsgm = 0;	/* last messages used memory (+d) chans*/
  u_long lmsgc = 0;	/* last messages entries (+d) chans*/
  int glinesc = 0;	/* glines count */
  int glinesm = 0;	/* glines used memory */
...
*/
  ...
  for (acptr = GlobalClientList; acptr; acptr = acptr->next)
    {
      if (MyConnect(acptr))
        {
          lc++;
          for (clink = acptr->confs; clink; clink = clink->next)
            lcc++;
        }
      else
        rc++;
      if (acptr->user)
        {
          us++;
          for (clink = acptr->user->invited; clink;
               clink = clink->next)
            usi++;
          for (clink = acptr->user->channel; clink;
               clink = clink->next)
            usc++;
          if (acptr->user->away)
            {
  ...
  for (chptr = channel; chptr; chptr = chptr->nextch)
    {
      ...
      for (clink = chptr->members; clink; clink = clink->next)
		{
      	  chu++;
		  if(clink->lmsg)
			{
			  lmsgc++;
			  lmsgm += strlen(clink->lmsg);
			}
		}
      for (clink = chptr->invites; clink; clink = clink->next)
        chi++;
      for (clink = chptr->banlist; clink; clink = clink->next)
        {
          chb++;
          chbm += (strlen(clink->value.cp)+1+sizeof(Link));

        	if (clink->value.banptr->banstr)
        		chbm += strlen(clink->value.banptr->banstr);
        	if (clink->value.banptr->who)
        		chbm += strlen(clink->value.banptr->who);
  ...
  for (aconf = glines; aconf; aconf = aconf->next)
    {
      glinesc++;
      glinesm += aconf->host ? strlen(aconf->host)+1 : 0;
      glinesm += aconf->user ? strlen(aconf->user)+1 : 0;      
      glinesm += aconf->passwd ? strlen(aconf->passwd)+1 : 0;
      glinesm += aconf->name ? strlen(aconf->name)+1 : 0;
      glinesm += sizeof(aConfItem);
    }
  /*
  ...
  sendto_one(cptr, ":%s %d %s :Channel members %d(%d) invite %d(%d), lmsg %d(%d)",
             me.name, RPL_STATSDEBUG, nick, chu, chu*sizeof(Link),
             chi, chi*sizeof(Link),lmsgc,lmsgm);
...
#endif
  sendto_one(cptr, ":%s %d %s :Gline list %d(%d)",
             me.name, RPL_STATSDEBUG, nick, glinesc, glinesm);
  tot += glinesm;  
  sendto_one(cptr, 
