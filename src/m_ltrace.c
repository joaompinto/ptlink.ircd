 ...
 *
 *   $Id: m_ltrace.c,v 1.4 2005/08/27 16:23:50 jpinto Exp $
 */
                ...
                char *parv[])
{
  ...
  if(MyClient(sptr))
    sendto_realops_flags(UMODE_SPY, "ltrace requested by %s (%s@%s) [%s]",
      ...
      switch(acptr->status)
        ...
        name=get_client_name(acptr, MASK_IP);
          ...
          sendto_one(sptr, form_str(RPL_TRACECONNECTING), me.name,
	             parv[0], c_class, name); /* could be not included yet */
          cnt++;
          break;
          case STAT_ME:
          break;
